...
public class ShowAlertActivity extends BaseActivity implements OnClickListener,
    ...
    void initData() {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                lists = DrugRemindDao.getInstance(ShowAlertActivity.this).queryByPatientId
						(patientId);
                if (null == lists) {
    ...
    public void getPatient() {
        ...
        new HttpManager().post(this,
                "drug/drugReminder/getDoseReminderListByPatientId",
                Alarms.class,
                interfaces,
                this, false, 1);
            ...
            if (null != alarm.data && alarm.data.size() > 0) {
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        DrugRemindDao.getInstance(ShowAlertActivity.this).clearByID("##112233##",
								patientId);
						...
						//	for (int j2 = 0; j2 < info.times_remind_oneday.size(); j2++) {
							if (!TextUtils.isEmpty(info.reminderTime1)&&info.reminderTime1.length
							()>=3) {
								...
								ala1.times = 0;
								ala1.eat = 0;
								ArrayList<Alarm> alarmupdates = (ArrayList<Alarm>) AlarmDao
								.getInstance(ShowAlertActivity.this).
							...
							if (!TextUtils.isEmpty(info.reminderTime2)&&info.reminderTime2.length
							()>=3) {
							...
							if (!TextUtils.isEmpty(info.reminderTime3)&&info.reminderTime3.length
							()>=3) {
							...
							if (!TextUtils.isEmpty(info.reminderTime4)&&info.reminderTime4.length
							()>=3){
								...
								ala4.times = 0;
								ala4.eat = 0;
								ArrayList<Alarm>	alarmupdates4 = (ArrayList<Alarm>) AlarmDao
								.getInstance(ShowAlertActivity.this).
							...
							List<DrugRemind> druglist = DrugRemindDao.getInstance
							(ShowAlertActivity.this).
									queryByids(info.id, patientId);
							AlarmBusiness.setAlarms(ShowAlertActivity.this, (List<Alarm>) alarms);
							if (druglist == null || druglist.size() == 0) {

								DrugRemindDao.getInstance(ShowAlertActivity.this).addOrUpdate
								(remind);
								for (Alarm alarm1 : remind.alarms) {
									AlarmDao.getInstance(ShowAlertActivity.this).createOrUpdate
									(alarm1);
